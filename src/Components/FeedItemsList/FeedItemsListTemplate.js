import I18n from '../../Internationalization.js'

const i18n = new I18n()

const FeedItemsListTemplate = data => `
${(data.feedItems.length <= 0) ? `<p>${i18n.t('FeedItemsList.NoUpdatesMessage')}</p>` : ''}
${data.feedItems.map(feedItem => `
    <article class="feed-item">
        ${(feedItem.link) ? `<a href="${feedItem.link}" target="_blank"  class="feed-item__link">` : '' }
            ${(feedItem.new) ? `<span class="feed-item__new">${i18n.t('FeedItemsList.New')}</span>` : ''}
            ${(feedItem.title) ? `<h1 class="feed-item__title">${feedItem.title}</h1>` : ''}
            ${(feedItem.description) ? `<div class="feed-item__description">${feedItem.description}</div>` : ''}
        ${(feedItem.link) ? `</a>` : '' }
        <footer class="feed-item__footer">
            ${(feedItem.pubDate) ? `<div><span class="feed-item__label">
            <vector-icon name="clock"></vector-icon>
                ${i18n.t('FeedItemsList.Date')}: </span>${feedItem.pubDate.toLocaleString()}</div>` : '' }
            ${(feedItem.categories.length > 0) ? `<div><span class="feed-item__label">${i18n.t('FeedItemsList.Categories')}: </span>${feedItem.categories.map(category => `
            <span>
                ${(category.domain) ? `<a href="${category.domain}" target="_blank">` : '' }
                    ${category.name}
                ${(feedItem.link) ? `</a>` : '' }
            </span>
            `).join(' | ')}</div>` : '' }
            ${(feedItem.comments) ? `<div><a href="${feedItem.comments}" target="_blank">${i18n.t('FeedItemsList.Comments')}</a></div>` : '' }
        </footer>
    </article>
`).join('')}`

export default FeedItemsListTemplate